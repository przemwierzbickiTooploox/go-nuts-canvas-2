...
class Coordinate {
  ...
  getDistanceTo(anotherCoordinate) {
...
class Node {
  constructor(position, color) {
    ...
    this.radius = random.range(12, 16);

    const directionX = random.rangeFloor(0, 2) ? 1 : -1;
    const directionY = random.rangeFloor(0, 2) ? 1 : -1;
    const vector = new Coordinate(
      random.range(0.25, 0.5) * directionX,
      random.range(0.25, 0.5) * directionY,
    );
  ...
  getDistanceTo(anotherNode) {
    return this.position.getDistanceTo(anotherNode.position);
  }
  ...
  return ({ context, width, height }) => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (let j = i + 1; j < nodes.length; j++) {
        const anotherNode = nodes[j];

        if (node.getDistanceTo(anotherNode) < CONFIG.DISTANCE * CONFIG.DISTANCE) {
          context.save();

          context.strokeStyle = node.color;
          context.beginPath();
          context.moveTo(node.position.x, node.position.y);
          context.lineTo(anotherNode.position.x, anotherNode.position.y);
          context.stroke();
...
const createPane = () => {
  ...
  folder.addInput(CONFIG, 'DISTANCE', { min: 50, max: 500, step: 25 });
};
